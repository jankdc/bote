...
The action opens (or updates) a "chore(release): version packages" PR that runs `npm run version-packages` (`changeset version`). Merging that PR bumps versions and clears the `.changeset/*.md` files.

### No pending changesets

The action runs `npm run release` (`changeset publish`), which publishes any workspace packages whose versions are ahead of npm. Native platform sub-packages are published via `@botejs/native`'s `prepublishOnly` (`napi prepublish -t npm --no-gh-release`).
