...
function recordsDoc(n: number): Uint8Array {
  ...
  for (let i = 0; i < n; i++) {
    ...
    parts.push(
      `{"id":${i},"name":"rec-${id}","detail":{"email":"u${i}@x.io","score":${i % 100}},"tags":["a","b","c"]}`,
    );
  }
  parts.push(']}');
...
function detailViewAt(idx: number): (c: Cur) => Promise<number> {
  return async (c) => {
    ...
    for await (const _ of c.iter('records', idx, { withKey: true })) {
      sink += 1;
    }
    for await (const _ of c.iter('records', idx, 'tags')) {
      sink += 1;
    }
    return sink;
  };
...
async function scatterGets(c: Cur): Promise<number> {
  let sink = 0;
  for (const i of SCATTER_IDXS) {
    sink += String(await c.get('items', i, 'name')).length;
  }
  return sink;
}
...
async function iterItems(c: Cur): Promise<number> {
  let n = 0;
  for await (const _ of c.iter('items')) {
    n += 1;
  }
  return n;
}
...
async function runScenarios(): Promise<void> {
  ...
  for (const s of scenarios) {
    ...
    if (!skipTime) {
      ...
      cols.push(
        fmtNs(time.cold).padStart(11),
        fmtNs(time.warm).padStart(11),
        speedup(time.cold, time.warm).padStart(9),
      );
    }
