...
async function loadHeapProfile(): Promise<{
  ...
  try {
    ...
    if (!native.heapProfileStart || !native.heapProfileStop || !native.heapProfilePeakBytes) return null
...
function renderTable(results: RunResult[]): void {
  ...
  const rustCell = (r: RunResult): string => {
    if (r.approach !== 'bote') return 'n/a'
    if (!r.rust_available) return 'n/a*'
